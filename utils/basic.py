# reference: https://github.com/aharley/simple_bev/blob/main/utils/basic.py
import numpy as np
import torch
...
def print_stats(name, tensor):
    shape = tensor.shape
    tensor = tensor.detach().cpu().numpy()
    print('%s (%s) min = %.2f, mean = %.2f, max = %.2f' % (name, tensor.dtype, np.min(tensor),
                                                           np.mean(tensor), np.max(tensor)), shape)
...
def gridcloud3d(B, Z, Y, X, norm=False, device='cuda'):
    ...
    # these are B x N   (1 x 320000)  320000=200*8*200
